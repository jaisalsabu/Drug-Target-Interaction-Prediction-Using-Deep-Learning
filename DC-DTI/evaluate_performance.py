...
if extension == 'csv':
    result_df = pd.read_csv(prediction_dir, header=None)
elif extension == 'tsv':
...
for dataset in test_names:
    tn, fp, fn, tp = confusion_matrix(result_df[dataset, "label"].dropna(), label_by_th(result_df[dataset, "predict"].dropna(), th)).ravel()
    ...
    if no_th:
        fpr, tpr, thresholds_AUC = roc_curve(result_df[dataset, "label"], result_df[dataset, "predict"])
        AUC = auc(fpr, tpr)
        precision, recall, thresholds = precision_recall_curve(result_df[dataset, "label"],
                                                               result_df[dataset, "predict"])
